...
DOCKER_CMD="docker"
# Non-interactive mode (set via -y/--yes flag or INSTALL_NONINTERACTIVE=true)
...
check_compatibility() {
    ...
    if [ "$AVAILABLE_SPACE" -lt 5 ]; then
        ...
        if [ "$NONINTERACTIVE" = true ]; then
            log_warning "Proceeding due to non-interactive mode"
        else
...
generate_env() {
    ...
    if [ -f .env ]; then
        ...
        if [ "$NONINTERACTIVE" = true ]; then
            log_info "Overwriting .env due to non-interactive mode"
        else
...
initialize_database() {
    ...
    # Wait for Postgres health via pg_isready inside the container
    log_info "Waiting for PostgreSQL to be ready (pg_isready)..."
    MAX_WAIT=30
    WAITED=0
    until $COMPOSE_CMD exec -T postgres pg_isready -U vpspanel >/dev/null 2>&1; do
        sleep 2
        WAITED=$((WAITED + 2))
        if [ $WAITED -ge $MAX_WAIT ]; then
            log_warning "PostgreSQL not ready after ${MAX_WAIT}s, continuing but init may fail"
            break
        fi
    done
...
# Database checker: list users and show key flags
db_check_users() {
    log_info "Checking database users..."
    # Try via postgres container using SQL
    if $COMPOSE_CMD ps postgres >/dev/null 2>&1; then
        $COMPOSE_CMD exec -T postgres psql -U vpspanel -d vpspanel -P pager=off -c \
"\
SELECT 
  id, email, username, role, 
  CASE WHEN is_active THEN 'yes' ELSE 'no' END AS active,
  CASE WHEN is_2fa_enabled THEN 'yes' ELSE 'no' END AS two_fa,
  to_char(coalesce(last_login, now()), 'YYYY-MM-DD HH24:MI') AS last_login
FROM users 
ORDER BY id;\
" || true
    else
        log_warning "Postgres container not found. Skipping direct SQL check."
    fi
}

# Reset admin credentials and disable 2FA
reset_admin_credentials() {
    local NEW_PASS="${ADMIN_NEW_PASSWORD:-admin123}"
    log_info "Resetting admin credentials (email=admin@example.com, password=${NEW_PASS}) and disabling 2FA..."
...
try:
    ...
    if not user:
        user = User(
            ...
            username="admin",
            hashed_password=get_password_hash("REPLACE_ME"),
        ...
        user.hashed_password = get_password_hash("REPLACE_ME")
        user.is_active = True
    ...
    db.close()
PY
    # Replace placeholder with shell value safely (run again with the real value)
    $COMPOSE_CMD exec -T backend python - <<PY
...
try:
    ...
    if u:
        u.hashed_password = get_password_hash(${NEW_PASS@Q})
        db.commit()
...
setup_firewall() {
    ...
    # Enable firewall
    if [ "$NONINTERACTIVE" = true ]; then
        log_info "Firewall rules added but not enabled (non-interactive mode)"
    else
...
install_ubuntu() {
    ...
    build_and_start
    initialize_database
    # Optional database check and remediation
    if [ "${RUN_DB_CHECK}" = true ]; then
        db_check_users
    fi
...
main() {
    ...
    if [ "$SYSTEM_TYPE" == "unknown" ]; then
        ...
        if [ "$NONINTERACTIVE" = true ]; then
            log_info "Proceeding due to non-interactive mode"
        else
